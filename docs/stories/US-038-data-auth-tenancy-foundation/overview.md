...
The repository has pure TypeScript domain slices and product contracts for class, fund, student, instructor, order, ledger, scenario, and realtime descriptor behavior. A first bounded Supabase/Drizzle foundation now exists with server-side Supabase Auth claim parsing, scoped database row parsing before result delivery for student fund, own-holding, TARA order, risk register, ledger, revealed macro narrative, revealed market metric, tracked metric, same-class leaderboard fund rows, and instructor owned-class/God Mode holding rows, injected student macro news, current-turn Driver/String dashboard, portfolio pyramid, TARA order-entry, current-turn dashboard server query executors, and a bounded instructor pending-order visibility server query executor over parsed RLS-backed rows, fixed-field safe authorization event serialization, server-only local database URL parsing for the RLS proof harness, a browser-safe Supabase auth environment parser for future login/logout client setup, a minimum Drizzle schema, a Supabase RLS migration with role-claim-aware student/instructor access policies, deterministic local fixtures, and `npm run test:integration:auth-tenancy`. No live database runtime, browser auth flow, hosted Supabase project, production migration path, or fully passing local RLS proof is wired yet.
