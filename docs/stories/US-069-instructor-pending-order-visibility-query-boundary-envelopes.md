# US-069 Instructor Pending-Order Visibility Query Boundary Envelopes
...
Instructors can receive a future server-query descriptor, result envelope, and validation failure envelope for pending-order visibility in one already-scoped class/current-month request. This slice wraps the existing status-only pending-order visibility snapshot without executing server queries, auth/session checks, RLS, database access, UI rendering, provider clients, or order detail delivery.
...
- A pure TypeScript function wraps an already-authorized pending-order visibility snapshot only when class and current-month scope match the descriptor.
...
- Descriptor, result, and validation failure payloads exclude target weights, estimated tax drag, order details, database rows, provider clients, UI state, and executed query metadata.
...
- API: none.
- Tables: none.
- Domain rules: result envelopes wrap only an already-authorized pending-order visibility snapshot matching the descriptor class and current month.
...
- `npm run test:unit -- app/domain/classes/pending-order-visibility.test.ts` — passed; 1 test file and 16 tests passed.
- `npm run validate:quick` — passed; 24 test files and 297 tests passed.
