...
- The MVP pure-domain student portfolio pyramid query descriptor, result envelope, and validation failure envelope record the future server-query boundary for one already-scoped viewer fund without emitting database rows, provider clients, other-fund holdings, instructor God Mode data, target weights, order details, estimated tax drag, ledger drafts, UI state, or executed query metadata.
