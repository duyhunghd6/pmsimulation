...
- Have bounded month-advance events processed through an injected worker executor that returns only aggregate turn-completion metadata plus a safe persistence receipt and dispatches successful completions through the injected server-only Supabase Realtime publisher with refresh-only metadata, while live Supabase reads/writes, hosted worker proof, hosted realtime proof, and processed-order mutation remain pending.
