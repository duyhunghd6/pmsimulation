...
- Have bounded month-advance events processed through an injected worker executor that returns only aggregate turn-completion metadata plus a safe persistence receipt, while live Supabase reads/writes, hosted worker proof, realtime publication, and processed-order mutation remain pending.
