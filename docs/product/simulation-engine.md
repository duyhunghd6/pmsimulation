...
The MVP pure-domain worker job envelope derives from the shared processing request, preserves the same trigger and class/month idempotency metadata, and excludes fund-level inputs before future Inngest, QStash, or other worker-provider code exists. The MVP pure-domain worker job result envelope derives from that provider-neutral job envelope and records an accepted worker-boundary receipt without executing a worker, dispatching provider jobs, writing database rows, publishing realtime payloads, or returning fund-level processing data.
...
The MVP pure-domain realtime authorized current-turn query result envelope derives from that query descriptor and wraps already-authorized student and instructor current-turn dashboard snapshots only when their class and current-month scope match, without executing server queries, auth/session checks, RLS, database access, UI refetching, provider subscriptions, or unscoped gameplay delivery. The MVP pure-domain realtime authorized current-turn query result validation failure envelope derives from missing or mismatched query result inputs and returns server-query-result validation errors without returning snapshots, database rows, provider clients, ledger drafts, fund processing keys, aggregate financial totals, UI state, or provider execution details.
