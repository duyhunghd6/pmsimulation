...
No Next.js app, browser auth flow, live database runtime, E2E tests, CI, deployment commands, worker execution, or realtime provider execution exists yet. `npm run test:integration:auth-tenancy` now exists for the US-038 local Supabase proof, but its local RLS test requires `AUTH_TENANCY_DATABASE_URL`. The approved MVP stack is Next.js App Router on Vercel with Supabase Auth/PostgreSQL/RLS/Realtime, Drizzle ORM, Inngest, Tailwind CSS, shadcn/ui, Apache ECharts, Tremor, plus integration, E2E, platform, and release proof as those layers are introduced by accepted stories.
...
When continuing autonomous full-stack work, follow `docs/stories/backlog.md` and select the earliest safe unimplemented layer. If US-038 local RLS execution is blocked only by missing `AUTH_TENANCY_DATABASE_URL`, do not keep stopping after parser-only slices; move to the Next.js app shell or browser auth flow as long as the slice exposes no protected gameplay data. Future rounds should progress through app shell/auth, server query execution, student UI, order submission, instructor UI, Inngest processing, Supabase Realtime, and release proof rather than repeatedly reporting those layers as skipped.
