...
import { createSupabaseInstructorClassAggregateAnalyticsRowReader } from './instructor-class-aggregate-analytics-supabase-reader';
...
describe('createSupabaseInstructorClassAggregateAnalyticsRowReader', () => {
  it('reads aggregate-safe row sets from scoped Supabase tables', async () => {
    ...
    const rows = await createSupabaseInstructorClassAggregateAnalyticsRowReader(
      createClient({ funds: fundRows, tara_orders: orderRows }, calls),
    ).readInstructorClassAggregateAnalyticsRows({ session: instructorSession, scope });

    expect(rows).toEqual({ funds: fundRows, orders: orderRows });
    expect(calls).toEqual([
      {
        table: 'funds',
        columns: 'id,class_id,current_aum,sharpe_ratio',
    ...
    expect(Object.prototype.hasOwnProperty.call(rows.funds[0], 'student_display_name')).toBe(false);
  ...
  it('fails closed without returning provider error payloads', async () => {
    ...
    await expect(
      createSupabaseInstructorClassAggregateAnalyticsRowReader(client).readInstructorClassAggregateAnalyticsRows({
        session: instructorSession,
        scope,
      }),
    ).rejects.toThrow('Supabase instructor class aggregate analytics read failed: funds');
  });
});
