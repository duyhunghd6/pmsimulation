...
function rowReader(
  rows: Partial<Awaited<ReturnType<InstructorLiveLeaderboardQueryRowReader['readInstructorLiveLeaderboardRows']>>> = {},
...
describe('executeInstructorLiveLeaderboardQuery', () => {
  it('returns a leaderboard-safe envelope for an instructor-scoped class', async () => {
    const result = await executeInstructorLiveLeaderboardQuery({
    ...
    expect(result).toEqual({
      ...
      value: {
        ...
        queryResultKey: 'class:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa:month:1:instructor-live-leaderboard-query:result-envelope',
        queryBoundary: 'server_query_result_boundary',
        queryDescriptorKey: 'class:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa:month:1:instructor-live-leaderboard-query',
  ...
  it('rejects non-instructor sessions before reading rows', async () => {
    ...
    await expect(executeInstructorLiveLeaderboardQuery({ session: studentSession, scope, rowReader: reader })).resolves.toEqual({
      ok: false,
  ...
  it('requires a current-month scope before reading rows', async () => {
    ...
    await expect(
      executeInstructorLiveLeaderboardQuery({ session: instructorSession, scope: { classId }, rowReader: reader }),
    ).resolves.toEqual({
      ok: false,
