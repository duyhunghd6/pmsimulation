...
import { createSupabaseInstructorLiveLeaderboardRowReader } from './instructor-live-leaderboard-supabase-reader';
...
describe('createSupabaseInstructorLiveLeaderboardRowReader', () => {
  it('reads leaderboard-safe row sets from scoped Supabase tables', async () => {
    ...
    const rows = await createSupabaseInstructorLiveLeaderboardRowReader(
      createClient({ funds: fundRows, tara_orders: orderRows }, calls),
    ).readInstructorLiveLeaderboardRows({ session: instructorSession, scope });
  ...
  it('fails closed without returning provider error payloads', async () => {
    ...
    await expect(
      createSupabaseInstructorLiveLeaderboardRowReader(client).readInstructorLiveLeaderboardRows({
        session: instructorSession,
        scope,
      }),
    ).rejects.toThrow('Supabase instructor live leaderboard read failed: funds');
  });
});
